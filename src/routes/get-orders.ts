...
export default async (req: Request, res: Response, next: NextFunction) => {
  ...
  knexDatabase.knexConfig.transaction(async (trx) => {
    ...
    const integration = await IntegrationService.getIntegrationByOrganizationId(organizationId as string)
    const url = `${ordersServiceUrl}/organization/${organizationId}/orders?type=${integration.type}&page=${page}&limit=20`
