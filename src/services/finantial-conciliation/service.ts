...
const getAffiliatesValuesByMonth = async (context: { organization_id: string; year_month: string }, trx: Transaction) => {
  try {
    const affiliateList = []
    const affiliates = await AffiliateHelper.getAffiliatesDict(context.organization_id, context.year_month)
    ...
    for (const affiliateId in affiliateIds) {
      let affiliateObj = {
      ...
      if (bankData) {
        ...
        affiliateObj.account = `${bankData.acount}-${bankData.account_digit}`
