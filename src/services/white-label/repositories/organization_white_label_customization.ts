...
const getWhiteLabelInfosByOrganizationId = async (organizationId?: string, trx?: Transaction, domain?: String) => {
  ...
  if (domain) {
    whiteLabelInfos = await (trx || knexDatabase.knexConfig)('organization_white_label_customization AS owlc')
      ...
      .select('owlc.primary_color', 'owlc.second_color', 'owlc.tertiary_color', 'owlc.logo', 'o.name', 'owlc.organization_id', 'owlc.custom_domain', 'owlc.personalized_terms_and_conditions')
      .first()
  } else {
    whiteLabelInfos = await (trx || knexDatabase.knexConfig)('organization_white_label_customization').where('organization_id', organizationId).andWhere('active', true).first().select()
  }
