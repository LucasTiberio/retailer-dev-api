...
describe('Services', () => {
    ...
    test('organization admin should handle user service to sale role', async done => {
        ...
        const invitedServiceMembers = await OrganizationService.inviteAffiliateServiceMembers(inviteAffiliatesInput, context, trx);

        const userInOrganizationService = await (trx || knexDatabase.knex)('users_organization_service_roles').first().select();

        const affiliateAnalystRole = await (trx || knexDatabase.knex)('service_roles').where('name', ServiceRoles.ANALYST).first().select('id');

        expect(userInOrganizationService.service_roles_id).toBe(affiliateAnalystRole.id);
        ...
        const userInOrganizationServiceChanged = await (trx || knexDatabase.knex)('users_organization_service_roles').first().select();

        const affiliateSaleRole = await (trx || knexDatabase.knex)('service_roles').where('name', ServiceRoles.SALE).first().select('id');

        expect(userInOrganizationServiceChanged.service_roles_id).toBe(affiliateSaleRole.id);
    ...
    test('organization admin not should handle user service to sale role with no available space', async done => {
        ...
        await OrganizationService.inviteAffiliateServiceMembers(inviteSaleInput, context, trx);
