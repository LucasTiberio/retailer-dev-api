    ...
    Object.keys(usersByRole).map(async (role) => {
      ...
      switch (role) {
        case ServiceRoles.ANALYST:
          if (usersToInvite.quantity > affiliateRules.maxAnalysts || usersToInvite.quantity + Number(usersInServiceOrganizationCounted) > affiliateRules.maxAnalysts)
            throw new Error(MESSAGE_ERROR_UPGRADE_PLAN)
          return
        case ServiceRoles.SALE:
          if (usersToInvite.quantity > affiliateRules.maxSales || usersToInvite.quantity + Number(usersInServiceOrganizationCounted) > affiliateRules.maxSales)
            throw new Error(MESSAGE_ERROR_UPGRADE_PLAN)
          return
        default:
          return null
...
const getOrganizationServicesByOrganizationId = async (userOrganizationId: string, organizationId: string) => {
...
const getOrganizationServicesById = async (userOrganizationServiceId: string) => {
...
const affiliatesCapacities = async (context: { organizationId: string }, trx: Transaction) => {
  const [serviceOrganization] = await serviceOrganizationByName(context.organizationId, Services.AFFILIATE, trx)
