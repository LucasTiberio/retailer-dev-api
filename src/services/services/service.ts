...
import {
  ...
  organizationServicesRolesByIdOneToOneLoader,
...
const createServiceInOrganization = async (
  serviceId: string,
...
const getServiceByName = async (serviceName: Services, trx: Transaction) => {
  const [serviceFound] = await (trx || knexDatabase.knex)("services")
...
const getServiceRolesById = async (serviceRolesId: string) => {
  const organizationServices = await organizationServicesRolesByIdLoader().load(
    serviceRolesId
  );
...
const getServiceRolesByOneId = async (serviceRolesId: string) => {
  const organizationServices = await organizationServicesRolesByIdOneToOneLoader().load(
    serviceRolesId
  );
...
const getServiceRolesByName = async (
  serviceRoleName: ServiceRoles | string,
  trx?: Transaction
) => {
  const [serviceRolesFound] = await (trx || knexDatabase.knex)("service_roles")
    .where("name", serviceRoleName)
    .select();
...
const serviceOrganizationByName = async (
  organizationId: string,
...
const getUserOrganizationServiceRole = async (
  usersOrganizationId: string,
  serviceOrganizationId: string,
  trx: Transaction
) => {
  const [userReactiveInOrganizationService] = await (trx || knexDatabase.knex)(
...
const getUserOrganizationServiceRoleName = async (
  usersOrganizationId: string,
  serviceOrganizationId: string,
  trx: Transaction
) => {
  const [userOrganizationServiceRoleName] = await (trx || knexDatabase.knex)(
...
const getUserOrganizationServiceRoleById = async (
...
const getOrganizationIdByUserOrganizationServiceRoleId = async (
  userOrganizationServiceRoleId: string,
  trx: Transaction
) => {
  const [userReactiveInOrganizationService] = await (trx || knexDatabase.knex)(
  ...
  return {
    ...
    organizationServiceId:
      userReactiveInOrganizationService.organization_services_id,
  };
};

const usersInServiceOrganizationCount = async (
  serviceRoleId: string,
  serviceOrganizationId: string,
  emails: string[],
  trx: Transaction
) => {
  const [usersInServiceOrganizationCounted] = await (trx || knexDatabase.knex)(
    "users_organization_service_roles AS uosr"
  )
    .innerJoin(
      "users_organizations AS uo",
...
const _usersInServiceOrganizationCountById = async (
  serviceRoleId: string,
  serviceOrganizationId: string,
  userOrganizationServiceRoleId: string,
  trx: Transaction
...
const verifyAffiliateMaxRules = async (
  ...
  users.forEach((user) => {
    ...
    Object.assign(usersByRole, {
      [user.role]: {
        quantity: quantity += 1,
        emails: usersByRole[user.role]
          ? [...usersByRole[user.role].emails, user.email]
          : [user.email],
      },
    });
  });
  ...
  await Promise.all(
    Object.keys(usersByRole).map(async (role) => {
      ...
      const usersInServiceOrganizationCounted = await usersInServiceOrganizationCount(
        affiliateService.id,
      ...
      switch (role) {
        case ServiceRoles.ANALYST:
          if (
            usersToInvite.quantity > affiliateRules.maxAnalysts ||
            usersToInvite.quantity + Number(usersInServiceOrganizationCounted) >
              affiliateRules.maxAnalysts
          )
            throw new Error(MESSAGE_ERROR_UPGRADE_PLAN);
          return;
        case ServiceRoles.SALE:
          if (
            usersToInvite.quantity > affiliateRules.maxSales ||
            usersToInvite.quantity + Number(usersInServiceOrganizationCounted) >
              affiliateRules.maxSales
          )
            throw new Error(MESSAGE_ERROR_UPGRADE_PLAN);
          return;
        default:
...
const _verifyAffiliateMaxRulesByUserOrganizationServiceId = async (
  ...
  const usersInServiceOrganizationCounted = await _usersInServiceOrganizationCountById(
    affiliateService.id,
    serviceOrganizationId,
  ...
  switch (role) {
    case ServiceRoles.ANALYST:
      if (
        1 > affiliateRules.maxAnalysts ||
        1 + Number(usersInServiceOrganizationCounted) >
          affiliateRules.maxAnalysts
      )
        throw new Error(MESSAGE_ERROR_UPGRADE_PLAN);
      return;
    case ServiceRoles.SALE:
      if (
        1 > affiliateRules.maxSales ||
        1 + Number(usersInServiceOrganizationCounted) > affiliateRules.maxSales
      )
        throw new Error(MESSAGE_ERROR_UPGRADE_PLAN);
      return;
    default:
...
const attachUserInOrganizationAffiliateService = async (
  ...
  const [serviceRoleSelected] = await (trx || knexDatabase.knex)(
    "service_roles"
  )
    .where("name", role)
    .select("id");
  ...
  const userOrganizationServiceRoleFound = await getUserOrganizationServiceRole(
    userOrganizationId,
    input.serviceOrganization.id,
    trx
  );

  if (userOrganizationServiceRoleFound) {
    const [userReactiveInOrganizationService] = await (
    ...
    return {
      ..._usersOrganizationServiceAdapter(userReactiveInOrganizationService),
      serviceId: input.serviceOrganization.service_id,
    };
  } else {
    const [userAddedInOrganizationService] = await (trx || knexDatabase.knex)(
      "users_organization_service_roles"
    )
      .insert({
        service_roles_id: serviceRoleSelected.id,
...
const listAffiliatesMembers = async (
  ...
  const [serviceOrganization] = await serviceOrganizationByName(
  ...
  if (input && input.name) {
    query = query.andWhereRaw(
      `(LOWER(usr.email) LIKE ? OR LOWER(usr.username) LIKE ?)`,
      [`%${input.name.toLowerCase()}%`, `%${input.name.toLowerCase()}%`]
    );
  }
...
const getUserInOrganizationService = async (
  getUserInOrganizationServicePayload: {
    userOrganizationId: string;
  },
  ...
  return usersInOrganizationService
    ? _usersOrganizationServiceAdapter(usersInOrganizationService)
    : null;
};

const getUserOrganizationServiceByServiceName = async (
  input: {
    serviceName: Services;
  },
  context: { client: IUserToken; userServiceOrganizationRolesId: string },
  trx: Transaction
) => {
  if (!context.client) throw new Error("token must be provided!");
...
const getUserInOrganizationServiceById = async (
  input: {
...
const isServiceAdmin = async (
  usersOrganizationId: string,
  serviceOrganizationId: string,
  trx: Transaction
) => {
  const [organizationServiceRole] = await (trx || knexDatabase.knex)(
    "users_organization_service_roles AS uosr"
  )
    .innerJoin("service_roles AS sr", "sr.id", "uosr.service_roles_id")
...
const handleServiceMembersRole = async (
  input: {
    serviceName: Services;
  ...
  const affiliateTeammateRules = await OrganizationRulesService.getAffiliateTeammateRules(
    context.organizationId
  );

  const serviceRoleFound = await getServiceRolesByName(input.serviceRole, trx);

  const [serviceOrganizationFound] = await serviceOrganizationByName(
    context.organizationId,
    serviceName,
    trx
  );
  ...
  const [userOrganizationServiceRoleUpdated] = await (trx || knexDatabase.knex)(
    "users_organization_service_roles"
  )
    .update({
      service_roles_id: serviceRoleFound.id,
    })
    .where("id", userOrganizationServiceRoleId)
    .returning("*");

  return {
    ..._usersOrganizationServiceAdapter(userOrganizationServiceRoleUpdated),
    serviceId: serviceOrganizationFound.service_id,
  };
};

const getServiceMemberById = async (
  userOrganizationId: string,
  organizationServiceId: string,
  trx: Transaction
) => {
  const [userOrganizationService] = await (trx || knexDatabase.knex)(
    "users_organization_service_roles"
  )
  ...
  return userOrganizationService
    ? _usersOrganizationServiceAdapter(userOrganizationService)
    : null;
};
...
const getOrganizationServicesByOrganizationId = async (
  ...
  ] = await OrganizationService.getUserOrganizationRole(userOrganizationId);
  ...
  if (isAdmin) {
    ...
    return allOrganizationServices.map((item: IServiceAdaptedFromDB) =>
      _serviceAdapter({ ...item, service_roles_id: serviceAdminRole.id })
    );
  }

  const organizationServices = await organizationServicesByOrganizationIdLoader().load(
    userOrganizationId
  );
...
const getOrganizationServicesById = async (
  ...
  const organizationServices = await userOrganizationServicesByIdLoader().load(
    userOrganizationServiceId
  );

  return organizationServices;
};

const verifyFirstSteps = async (
  userServiceOrganizationId: string,
  bankDataId: string
) => {
  const bankData = await BankDataService.getBankDataById(bankDataId);

  const hasLinkGenerated = await userOrganizationServicesHasLinkGeneratedByIdLoader().load(
    userServiceOrganizationId
  );

  return !(!!hasLinkGenerated && !!bankData);
};

const getUserInOrganizationServiceByUserOrganizationId = async (
  input: { usersOrganizationId: string; activity?: boolean },
  trx: Transaction
) => {
  let query = (trx || knexDatabase.knex)(
    "users_organization_service_roles"
...
const getOrganizationServicesByOrganization = async (
  organizationId: string,
  trx: Transaction
) => {
...
const inativeServiceMembersById = async (
  userOrganizationServiceIds: string[],
  trx: Transaction
) => {
...
const getOrganizationServiceByServiceName = async (
  input: {
    service: Services;
    organizationId: string;
  },
...
const affiliatesCapacities = async (
  ...
  return {
    analyst: {
      ...
      used:
        Number(
          query.find((item) => item.name === ServiceRoles.ANALYST)?.count
        ) || 0,
    },
    sale:
      affiliateTeammateRules.maxSales > 0
        ? {
            ...
            used:
              Number(
                query.find((item) => item.name === ServiceRoles.SALE)?.count
              ) || 0,
          }
        : null,
  };
...
export default {
  ...
  _verifyAffiliateMaxRulesByUserOrganizationServiceId,
