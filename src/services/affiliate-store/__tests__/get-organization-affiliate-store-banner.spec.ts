...
describe('Organization', () => {
  ...
  it('organization admin should add organization affiliate store banner', async (done) => {
    imgGen.generateImage(50, 50, 80, async function (err: Error, image: any) {
      ...
      const organizationAffiliateStoreBanner = await service.getOrganizationAffiliateStoreBanner({ organizationId: organizationInserted.id }, trx)

      expect(organizationAffiliateStoreBanner).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            id: organizationAffiliateStore.id,
            organizationAffiliateStoreId: organizationAffiliateStore.organizationAffiliateStoreId,
            url: organizationAffiliateStore.url,
          }),
        ])
      )
