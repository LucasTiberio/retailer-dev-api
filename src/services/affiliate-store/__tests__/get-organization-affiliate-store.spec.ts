...
describe('Organization', () => {
  ...
  it('organization admin should get organization affiliate store', async (done) => {
    const input = {
      active: true,
      shelfId: Faker.random.uuid(),
    }

    await service.handleOrganizationAffiliateStore(input, { organizationId: organizationInserted.id }, trx)

    const organizationAffiliateStore = await service.getOrganizationAffiliateStore({ organizationId: organizationInserted.id }, trx)
