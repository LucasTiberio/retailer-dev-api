...
import { ORDER_MOMENTS, buildVerifyVtexSecretsUrl, buildOrderHookVtexUrl, buildCreateCampaignVtexUrl, buildGetCategoriesThreeVtexUrl } from './helpers'
...
const verifyVtexSecrets = async (secrets: IVtexSecrets) => {
  try {
    const data = await axios.get(buildVerifyVtexSecretsUrl(secrets.accountName), {
      params: {
...
const attachVtexSecrets = async (
  ...
  const [verifySecretExists] = await (trx || knexDatabase)('organization_vtex_secrets').where('store_name', accountName).andWhere('organization_id', organizationId).select()
...
const getSecretsByOrganizationId = async (organizationId: string, trx: Transaction) => {
  const [verifySecretExists] = await (trx || knexDatabase)('organization_integration_secrets AS ois')
...
const getVtexDepartments = async (getVtexDepartmentsPayload: { organizationId: string }, userToken: IUserToken, trx: Transaction) => {
  ...
  const response = await axios.get(buildGetCategoriesThreeVtexUrl(storeSecrets?.storeName), {
    headers: {
      'content-type': 'Content-Type',
    },
  })
...
const getVtexDepartmentsCommissions = async (context: { organizationId: string; client: IUserToken }, trx: Transaction) => {
...
const getComissionsDepartmentsByOrganizationId = async (organizationId: string, trx: Transaction) => {
  const vtexComissions = await (trx || knexDatabase)('organization_vtex_comission').where('organization_id', organizationId).select()
...
const getVtexCommissionInfosByAffiliateIdAndDepartmentId = async (
  ...
  const vtexCommissionPayDay = await AffiliateService.getTimeToPayCommissionById({ organizationId: userOrganizationService.organizationId }, trx)
  ...
  if (vtexCommission.find((item) => item.percentage === null)) {
    const defaultCommission = await AffiliateService.getDefaultCommissionByOrganizationServiceId(userOrganizationService.organizationServiceId, trx)
