...
const sendRecurrencyTransaction = async (
  ...
  try {
    ...
    if (!affiliateRules.providers.filter((provider: { name: Integrations }) => provider.name === Integrations.VTEX)[0].status) {
...
const createEnterpriseRecurrencyTransaction = async (input: { zipCode: string; number: string; complement?: string; cpfCnpj: string; valueCents: number; organizationId: string }) => {
  const mutation = `
