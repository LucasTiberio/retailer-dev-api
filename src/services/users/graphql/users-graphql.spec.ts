...
describe('users graphql', () => {
    ...
    test("should create new user", async done => {
        ...
        const userOnDb = await database.knexTest('users').where("id", signUpResponse.body.data.signUp.id).select();
    ...
    describe("tets with user before created graphql", () => {
        ...
        test("user should verify your sign up graphql", async done => {
            ...
            const [userFromDb] = await database.knexTest('users').where('id', signUpCreated.id).select('verification_hash');
            ...
            const userOnDb = await database.knexTest('users').where('id', signUpCreated.id).select();
        ...
        test("user should recovery and change your password", async done => {
            ...
            const [userFound] = await database.knexTest('users').where('id', signUpCreated.id).select();
