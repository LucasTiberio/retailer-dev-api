...
const resolvers: IResolvers = {
  Mutation: {
    ...
    handleDefaultCommission: (_, { input }, { organizationId, client }) => {
      return knexDatabase.knex.transaction((trx: Transaction) => {
        return service.handleDefaultommission(
          input,
          { organizationId, client },
          trx
        );
    ...
    affiliateGenerateShortenerUrl: (_, attrs, { client, organizationId }) => {
      const { input } = attrs;
      return knexDatabase.knex.transaction((trx: Transaction) => {
        return service.generateShortenerUrl(
          input,
    ...
    createAffiliateBankValues: (
      _,
      attrs,
      { client, organizationId, userServiceOrganizationRolesId }
    ) => {
      const { input } = attrs;
      return knexDatabase.knex.transaction((trx: Transaction) => {
  ...
  Query: {
    ...
    listAffiliateShorterUrl: (_, attrs, { client }) => {
      const { input } = attrs;
      return knexDatabase.knex.transaction((trx: Transaction) => {
        return service.getShorterUrlByUserOrganizationServiceId(
          input,
          client,
          trx
        );
  ...
  UserOrganizationServiceRolesUrlShortener: {
    ...
    userOrganizationService: async (obj) => {
      return ServicesService.getOrganizationServicesById(
        obj.usersOrganizationServiceRolesId
      );
    },
  },
};
