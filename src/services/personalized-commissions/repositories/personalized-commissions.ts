...
const getCommissionTypeByOrganizationId = async (organization_id: string, type: string, trx: Transaction) => {
  return await (trx || knexDatabase.knexConfig)('organization_commission_order').where('organization_id', organization_id).andWhere('type', type).first().select()
}

const getCommissionOrderByOrganizationId = async (organization_id: string, trx: Transaction) => {
  return await (trx || knexDatabase.knexConfig)('organization_commission_order').where('organization_id', organization_id).select().orderBy('order', 'asc')
}
...
const findOrUpdate = async (input: CommissionsOrder, organization_id: string, trx: Transaction) => {
  ...
  let query = (trx || knexDatabase.knexConfig)('organization_commission_order')
