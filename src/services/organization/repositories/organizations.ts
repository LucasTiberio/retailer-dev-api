...
const getOrganizationById = async (organizationId: string, trx: Transaction) => {
  const organization = await (trx || knexDatabase.knex)('organizations').where('id', organizationId).first().select()
...
const handleOrganizationDomainById = async (domain: string, organizationId: string, trx: Transaction) => {
  const [organizationHandled] = await (trx || knexDatabase.knex)('organizations').update('domain', domain).where('id', organizationId).returning('*')
