import {
  ...
  IResponseInvitePayload,
  IFindUsersAttributes,
} from "./types";
...
import {
  ...
  MESSAGE_ERROR_USER_PENDENT_ORGANIZATION_INVITE,
...
import {
  ...
  _usersOrganizationsAdapter,
  _usersOrganizationsRolesAdapter,
} from "./adapters";
import {
  organizationByIdLoader,
  organizationByUserIdLoader,
  organizationRoleByUserIdLoader,
  organizationHasMemberLoader,
...
const attachOrganizationAditionalInfos = async (
  ...
  const [attachedOrganizationInfosId] = await (trx || knexDatabase.knex)(
    "organization_additional_infos"
  )
    .insert({
      segment,
...
const createOrganization = async (
  ...
  try {
    ...
    const affiliateServiceFound = await ServicesService.getServiceByName(
      Services.AFFILIATE,
      trx
    );

    if (!affiliateServiceFound)
      throw new Error("Affiliate service doesnt exists.");

    await ServicesService.createServiceInOrganization(
      affiliateServiceFound.id,
      organizationCreated.id,
      context.client,
      trx
    );
    ...
    await setCurrentOrganization(
      ...
      { client: context.client, redisClient: context.redisClient },
      trx
    );
...
const organizationRolesAttach = async (
  ...
  const [organizationRoleId] = await (trx || knexDatabase.knex)(
...
const organizationRoleByName = async (
  roleName: OrganizationRoles,
  trx: Transaction
) => {
  const [organizationRole] = await (trx || knexDatabase.knex)(
    "organization_roles"
  )
    .where("name", roleName)
    .select("id");
...
const listTeammates = async (
  ...
  const teammates = await (trx || knexDatabase.knex)(
...
const userTeammatesOrganizationCount = async (
  organizationId: string,
  userId: string,
  emails: string[],
  trx: Transaction
) => {
  const [userOrganizationCreatedId] = await (trx || knexDatabase.knex)(
    "users_organizations AS uo"
  )
    .innerJoin(
      "users_organization_roles AS uor",
...
const userTeammatesOrganizationCountByUserOrganizationId = async (
  organizationId: string,
  userId: string,
  trx: Transaction
) => {
...
const inviteTeammates = async (
  ...
  const currentTeammates = await userTeammatesOrganizationCount(
    context.organizationId,
    context.client.id,
    input.emails,
    trx
  ...
  if (
    uniqueEmails.length > maxTeammates ||
    Number(currentTeammates) + uniqueEmails.length > maxTeammates
  )
  ...
  try {
    let users = await Promise.all(
      uniqueEmails.map(async (item) => {
        let hashToVerify = await common.encryptSHA256(
          JSON.stringify({ item, timestamp: +new Date() })
        );

        const userEmailFound = await UserService.getUserByEmail(item, trx);

        if (userEmailFound) {
          const usersOrganizationFound = await getUserOrganizationByIds(
            userEmailFound.id,
            context.organizationId,
            trx
          );
          ...
          if (usersOrganizationFound) {
            ...
            const userInOrganizationService = await ServicesService.getUserInOrganizationServiceByUserOrganizationId(
              { usersOrganizationId: usersOrganizationFound.id },
              trx
            );
...
const getUserOrganizationByUserOrganizationId = async (
  usersOrganizationId: string,
  trx: Transaction
) => {
  const userInOrganizationServices = await (trx || knexDatabase.knex)(
    "users_organizations"
...
const inviteAffiliateServiceMembers = async (
  ...
  try {
    await ServicesService.verifyAffiliateMaxRules(
      input.users,
    ...
    return await Promise.all(
      input.users.map(async (item) => {
        ...
        if (userEmail) {
          ...
          if (usersOrganizationFound) {
            const [userOrganizationUpdated] = await (trx || knexDatabase.knex)(
              "users_organizations"
            )
              .update({
                invite_status: OrganizationInviteStatus.PENDENT,
            ...
            const memberOrganizationRole = await getOrganizationRoleByName(
...
const getOrganizationRoleByName = async (
  ...
  const [organizationRole] = await (trx || knexDatabase.knex)(
    ...
    .where("name", organizationRoleName)
    .select();

  return organizationRole;
};

const changeOrganizationAdminToMember = async (
...
const responseInvite = async (
  responseInvitePayload: IResponseInvitePayload,
  trx: Transaction
) => {
  ...
  try {
    ...
    await (trx || knexDatabase.knex)("users_organization_service_roles")
      .update({
        active:
          responseInvitePayload.response === OrganizationInviteStatus.ACCEPT,
      })
...
const userOrganizationInviteStatus = async (
  userId: string,
  organizationId: string,
  trx: Transaction
) => {
  const [userOrganizationInviteStatusFound] = await (trx || knexDatabase.knex)(
    "users_organizations"
  )
...
const getUserOrganizationById = async (
  userOrganizationId: string,
  trx?: Transaction
) => {
  const [userOrganization] = await (trx || knexDatabase.knex)(
    "users_organizations"
  )
...
const getOrganizationByName = async (
  ...
  const [organization] = await (trx || knexDatabase.knexTest)("organizations")
...
const getOrganizationRoleId = async (
  organizationRoleName: OrganizationRoles,
  trx: Transaction
) => {
  const [organizationRole] = await (trx || knexDatabase.knex)(
...
const getOrganizationRoleById = async (
...
const listUsersInOrganization = async (
  ...
  if (name) {
    query = query.andWhereRaw(
      `(LOWER(email) LIKE ? OR LOWER(username) LIKE ?)`,
  ...
  const result = await query
    .select("uo.*", "uor.organization_role_id")
...
const getUserOrganizationByIds = async (
  userId: string,
  organizationId: string,
  trx: Transaction
) => {
...
const getUserOrganizationRoleById = async (
  userOrganizationId: string,
  trx: Transaction
) => {
  const [userOrganizationRole] = await (trx || knexDatabase.knex)(
    "users_organization_roles"
  )
...
const handleUserPermissionInOrganization = async (
  ...
  if (
    permission === OrganizationRoles.MEMBER &&
    !(await isFounder(context.organizationId, context.client.id, trx))
  ) {
    throw new Error(
      "Only founder can remove admin permission from organization."
    );
  }

  try {
    const userOrganization = await getUserOrganizationByIds(
      userId,
      context.organizationId,
      trx
    );
    ...
    const [userPermissionUpdated] = await (trx || knexDatabase.knex)(
      "users_organization_roles"
    )
...
const listMyOrganizations = async (userToken: IUserToken, trx: Transaction) => {
  ...
  try {
    const organizations = await (trx || knexDatabase.knex)(
      "users_organizations AS uo"
    )
...
const getUserOrganizationRole = async (userOrganizationId: string) => {
  const userOrganizationRole = await organizationRoleByUserIdLoader().load(
    userOrganizationId
  );
...
const verifyOrganizationHasMember = async (organizationId: string) => {
  const userOrganizationRole = await organizationHasMemberLoader().load(
    organizationId
  );
...
const organizationUploadImage = async (
  ...
  if (
    !mimetype.match(/\/png/gi)?.length &&
    !mimetype.match(/\/jpg/gi)?.length &&
    !mimetype.match(/\/jpeg/gi)?.length
  ) {
  ...
  const [organizationAttachLogo] = await (trx || knexDatabase.knex)(
    "organizations"
  )
    .where("id", context.organizationId)
    .update({
...
const setCurrentOrganization = async (
  currentOrganizationPayload: { organizationId: string | null },
  context: { client: IUserToken; redisClient: RedisClient },
  trx: Transaction
) => {
  ...
  const isUserOrganization = await getUserOrganizationByIds(
    context.client.id,
  ...
  try {
    const currentOrganization = await context.redisClient.setAsync(
      context.client.id,
      currentOrganizationPayload.organizationId
    );
...
const handleServiceMembersActivity = async (
  ...
  try {
    ...
    const userOrganizationServiceRoleFound = await (trx || knexDatabase.knex)(
    ...
    await (trx || knexDatabase.knex)("users_organization_service_roles")
      ...
      .where("id", userOrganizationServiceRoleFound.id);
...
const getUserByUserOrganizationId = async (
  userOrganizationId: string,
  trx: Transaction
) => {
  const user = await (trx || knexDatabase.knex)("users_organizations AS uo")
...
const handleTeammatesActivity = async (
  ...
  if (activity) {
    ...
    const currentTeammates = await userTeammatesOrganizationCountByUserOrganizationId(
      context.organizationId,
      context.client.id,
      trx
    );
  ...
  const [userOrganizationInatived] = await (trx || knexDatabase.knex)(
    "users_organizations"
  )
    .update({ active: activity })
    .where("id", userOrganizationId)
...
const reinviteServiceMember = async (
  ...
  try {
    const usersOrganizationFound = await (trx || knexDatabase.knex)(
      "users_organizations AS uo"
    )
      .innerJoin("users AS usr", "usr.id", "uo.user_id")
      .innerJoin("organizations AS org", "org.id", "uo.organization_id")
    ...
    if (!usersOrganizationFound.invite_hash)
      throw new Error(MESSAGE_ERROR_USER_ALREADY_REPLIED_INVITE);
...
const teammatesCapacities = async (
  ...
  const organizationAdminRole = await getOrganizationRoleByName(
    OrganizationRoles.ADMIN,
    trx
  );
...
const organizationHasBillingPendency = async (organizationId: string) => {
  try {
    ...
    const organizationFound = await knexDatabase
      .knex("organizations")
      .where("id", organizationId)
      .first()
      .select();
