...
describe("handle affiliate activity teammates", () => {
    ...
    test('user organization admin should inative affiliates', async done => {
        ...
        let inativeServiceMembersIds = {userOrganizationId: invitedServiceMembers[0].id, activity: false, service: Services.AFFILIATE};

        await service.handleServiceMembersActivity(inativeServiceMembersIds, context, trx);
        ...
        const usersInOrganizationService = await (trx || knexDatabase.knex)('users_organization_service_roles').where('active', false).select();

        expect(usersInOrganizationService).toHaveLength(1);

        const usersInOrganizationServiceActive = await (trx || knexDatabase.knex)('users_organization_service_roles').where('active', true).select();
    ...
    test('user organization admin should active affiliates', async done => {
        ...
        let activeServiceMembersIds = {userOrganizationId: invitedServiceMembers[0].id, activity: true, service: Services.AFFILIATE};

        await service.handleServiceMembersActivity(activeServiceMembersIds, context, trx);
