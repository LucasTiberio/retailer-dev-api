...
const TEAMMATES_CAPACITIES = `
    query teammatesCapacities{
        teammatesCapacities{
            teammates{
                total
                used
...
describe('invite teammates graphql', () => {
    ...
    const inviteTeammatesInput = {
        emails: Array(3).fill(0).map(() => Faker.internet.email())
    }
    ...
    test("user organization admin should invite teammates below plan limit - graphql", async done => {
        ...
        const teammatesCapacitiesResponse = await request
        .post('/graphql')
        .set('content-type', 'application/json')
        .set('x-api-token', userToken)
        .send({
        'query': TEAMMATES_CAPACITIES, 
        'variables': {
                input: inviteTeammatesInput
            }
        });

        expect(teammatesCapacitiesResponse.statusCode).toBe(200);

        expect(teammatesCapacitiesResponse.body.data.teammatesCapacities).toEqual(expect.objectContaining({
            teammates: {
                total: 5,
                used: 3
            }
