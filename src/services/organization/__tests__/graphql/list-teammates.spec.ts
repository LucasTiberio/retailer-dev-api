...
describe('invite teammates graphql', () => {
    ...
    let invitedTeammates: IUserOrganizationAdapted[];
    ...
    beforeEach(async () => {
        ...
        invitedTeammates = inviteTeammatesResponse.body.data.inviteTeammates;
    });
    ...
    test("user organization admin should invite teammates below plan limit - graphql", async done => {
        ...
        const listTeammatesResponse = await request
        .post('/graphql')
        .set('content-type', 'application/json')
        .set('x-api-token', userToken)
        .send({
        'query': LIST_TEAMMATES
        });

        expect(listTeammatesResponse.statusCode).toBe(200);

        expect(listTeammatesResponse.body.data.listTeammates).toHaveLength(6);
