let permissions = [{ name: 'commission' }, { name: 'orders' }, { name: 'generateLink' }, { name: 'payments' }, { name: 'members' }, { name: 'affiliate-settings' }]
...
exports.seed = async function (knex) {
  return Promise.all([
    knex('service_roles')
      .insert([{ name: SERVICE_ADMIN }, { name: SERVICE_RESPONSIBLE }, { name: SERVICE_ANALYST }, { name: SERVICE_SALE }])
      .returning('*'),
    knex('permissions').insert(permissions).returning('*'),
    knex('services')
      .insert([{ name: AFFILIATE_SERVICE }, { name: 'teste' }])
      .returning('*'),
  ]).then((item) => {
